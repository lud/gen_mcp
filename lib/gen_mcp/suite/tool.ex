defmodule GenMCP.Suite.Tool do
  ...
  A tool that runs slow work inline answers from `c:call/3` like any other, and
  it gets its own worker process for the request, so it can take as long as the
  work takes. Open the stream first with
  `GenMCP.Mux.Channel.start_stream/1` so the transport keeps writing keepalives
  while the work runs, which holds the connection through proxy and client idle
  timeouts:
  ...
  The tool is inside `c:call/3` for the whole of the work, and it learns of a
  client disconnect once the work returns. When the work is costly enough to be
  worth abandoning the moment the client goes away, hand it to a task under your
  own `Task.Supervisor` and wait for it as a streaming tool: the worker sits idle
  in `c:handle_message/4`, so it acts on the disconnect immediately, and
  `c:handle_close/3` can shut the task down.
  ...
  A tool that takes a long time before it can answer calls
  `GenMCP.Mux.Channel.start_stream/1` first, so the transport keeps the
  connection alive while it works. See the "Slow tools" section of the module
  doc.
  """
