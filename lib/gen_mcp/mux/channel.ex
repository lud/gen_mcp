defmodule GenMCP.Mux.Channel do
  ...
  * `send_result/2` and `send_error/2` deliver the final reply for the request.
  ...
  A channel is `:open` from the moment the transport builds it until something
  closes it, and every send works while it is open. A handler ends the response
  itself with `close/1`, which is how it stops a long-lived stream from the
  server side. `set_closed/1` and `as_closed/1` mark a channel closed without
  telling the client, which is what the transport does once the client has
  already disconnected. A send on a closed channel returns `{:error, :closed}`.

  ### Streaming

  The response is a single JSON reply until there is something to stream on it,
  and it becomes an SSE stream from the first thing sent. `send_progress/4`,
  `send_log/4` and `send_notification/2` each start the stream as they send, and
  `start_stream/1` starts it on its own. Reach for `start_stream/1` when a
  handler is about to spend a long time working before it has anything to send:
  the transport writes periodic keepalives on an open stream, and those keep a
  proxy or a client from dropping the connection as idle.
  ...
  `close/1` ends the response, whether it is an open stream or a reply still to
  be written.
  """
  ...
  * `logger` - an optional logger name string attached to the record.
  ...
  A handler with something to send starts the stream as it sends, through
  `send_progress/4`, `send_log/4` or `send_notification/2`. Call `start_stream/1`
  when the response has to stay open before there is anything to put on it, which
  is the case for a handler doing slow work inline. The transport writes periodic
  keepalives on an open stream, so the connection survives work that takes a
  while:
  ...
  The response is then `text/event-stream` and the result is delivered as its last
  event. Calling this more than once, or after something has already been sent,
  leaves the one stream in place.

  The worker stays inside the callback for the whole of the slow work, so it
  observes a client disconnect once that work returns. To abandon expensive work
  as soon as the client goes away, return `{:stream, state}` and await a
  supervised task instead, as `c:GenMCP.Suite.Tool.handle_message/4` shows.

  Returns `:ok`, or `{:error, :closed}` once the channel is closed. See `close/1`
  to end the response.
  """
  def start_stream(%{status: :closed}) do
    {:error, :closed}
  end
  ...
  `{:error, :closed}`. This ends the response whether or not it ever became a
  stream.
