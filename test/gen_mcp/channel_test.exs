defmodule GenMCP.ChannelTest do
  ...
  describe "start_stream/1 (spec 023)" do
    test "tells the client process to switch the response to a stream" do
      # The same message `GenMCP.Server` emits on a `{:stream, state}` return,
      # without the return tuple — so a handler can open the stream and still
      # answer terminally from the callback it is already in.
      channel = build_channel()

      assert :ok = Channel.start_stream(channel)
      assert_receive {:"$gen_mcp", :stream}
    end

    test "repeat calls are allowed (the relay opens the stream once)" do
      channel = build_channel()
    ...
    test "returns an error and sends nothing on a closed channel" do
      channel = Channel.as_closed(build_channel())

      assert {:error, :closed} = Channel.start_stream(channel)
      refute_receive {:"$gen_mcp", :stream}
    end
  end

  describe "status (spec 023)" do
    # The status tracks the channel lifecycle only: a channel is `:open` until
    # something closes it. Response mode (direct JSON vs SSE) is not on the
    # struct — it lives in the conn process, and any notification send can flip
    # it without the struct knowing, so the struct cannot track it truthfully.
    test "a request-built channel starts open" do
      req = %MCP.CallToolRequest{id: 1, params: %{}}

      assert %Channel{status: :open} = Channel.from_request(nil, req, %{})
    end

    test "a pid-built channel starts open" do
      assert %Channel{status: :open} = Channel.for_pid(self())
    end

    test "set_closed/1 closes without notifying the client" do
      assert %Channel{status: :closed} = Channel.set_closed(build_channel())
      refute_receive {:"$gen_mcp", _}
    end

    test "as_closed/1 closes and drops the client" do
      assert %Channel{status: :closed, client: nil} = Channel.as_closed(build_channel())
      refute_receive {:"$gen_mcp", _}
    end

    test "close/1 ends the response from an open channel, whether or not a stream was started" do
      channel = build_channel()

      assert {:ok, %Channel{status: :closed} = closed} = Channel.close(channel)
      assert_receive {:"$gen_mcp", :close}

      assert {:error, :closed} = Channel.close(closed)
    end

    test "close/1 ends the response after start_stream/1 too" do
      channel = build_channel()

      assert :ok = Channel.start_stream(channel)
      assert_receive {:"$gen_mcp", :stream}

      assert {:ok, %Channel{status: :closed}} = Channel.close(channel)
      assert_receive {:"$gen_mcp", :close}
    end

    test "every send function refuses a closed channel" do
      channel = Channel.as_closed(%{build_channel() | log_level: :debug, progress_token: "tok"})

      assert {:error, :closed} = Channel.send_progress(channel, 1, 2)
      assert {:error, :closed} = Channel.send_log(channel, :error, "nope")
      assert {:error, :closed} = Channel.send_notification(channel, %{method: "x"})
      assert {:error, :closed} = Channel.send_result(channel, %{})
      assert {:error, :closed} = Channel.send_error(channel, %{})
      assert {:error, :closed} = Channel.start_stream(channel)
      assert {:error, :closed} = Channel.close(channel)

      refute_receive {:"$gen_mcp", _}
      refute_receive {:"$gen_mcp", _, _}
    end

    test "every send function accepts an open channel" do
      channel = %{build_channel() | log_level: :debug, progress_token: "tok"}

      assert :ok = Channel.send_progress(channel, 1, 2)
      assert :ok = Channel.send_log(channel, :error, "yes")
      assert :ok = Channel.send_notification(channel, %{method: "x"})
      assert {:ok, _} = Channel.send_result(channel, %{})
      assert {:ok, _} = Channel.send_error(channel, %{})
      assert :ok = Channel.start_stream(channel)
      assert {:ok, _} = Channel.close(channel)
    end
  end
