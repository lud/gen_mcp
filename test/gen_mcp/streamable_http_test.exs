...
defmodule GenMCP.StreamableHTTPTest do
  ...
  # Shortens the relay's keepalive interval for one test, restoring the default
  # afterwards. The real interval is 25s, which no test can wait for.
  defp short_keepalive(millis) do
    previous = Application.get_env(:gen_mcp, :stream_keepalive_timeout)
    Application.put_env(:gen_mcp, :stream_keepalive_timeout, millis)

    on_exit(fn ->
      case previous do
        nil -> Application.delete_env(:gen_mcp, :stream_keepalive_timeout)
        value -> Application.put_env(:gen_mcp, :stream_keepalive_timeout, value)
      end
    end)
  end

  # Decodes the JSON-RPC payloads out of raw SSE chunks. A keepalive is chunked
  # as the bare comment line `":keepalive\n"`, with no blank line after it, so it
  # fuses with whatever event follows when splitting on the event separator —
  # drop those first, then split.
  defp sse_payloads(chunks) do
    chunks
    |> Enum.join()
    |> String.replace(":keepalive\n", "")
    |> String.split("\n\n", trim: true)
    |> Enum.map(fn event ->
  ...
  describe "Channel.start_stream/1 (spec 023)" do
    # A handler doing slow work inline answers terminally, so it never returns
    # `{:stream, state}` and the response would stay a direct JSON reply written
    # only when the work finishes — no bytes, and so no keepalives, for the whole
    # call. `start_stream/1` commits the response to SSE from inside the callback
    # the handler is already in, without changing what it returns.

    test "a handler that opens the stream then answers terminally responds over SSE" do
      expect_request(fn %MCP.CallToolRequest{id: 1}, channel, _state ->
        assert :ok = Channel.start_stream(channel)
        {:result, MCP.call_tool_result(text: "slow answer")}
      end)
      ...
      assert %{
               "id" => 1,
               "jsonrpc" => "2.0",
               "result" => %{"content" => [%{"type" => "text", "text" => "slow answer"}]}
             } = JSV.Codec.decode!(json)
    end

    test "keepalives flow while the handler blocks after start_stream/1" do
      # The point of the whole thing: the keepalive timer lives in the conn
      # process, so it keeps chunking while the worker is stuck inside one
      # callback. The interval is shortened to keep the test fast.
      short_keepalive(100)
      test_pid = self()

      expect_request(fn %MCP.CallToolRequest{id: 2}, channel, _state ->
        assert :ok = Channel.start_stream(channel)
        send(test_pid, {:working, self()})

        receive do
          :finish -> :ok
        end

        {:result, MCP.call_tool_result(text: "done blocking")}
      end)
      ...
      # Nothing has been written yet, so the first bytes on the wire can only be
      # a keepalive produced while the worker is blocked.
      assert {:ok, [data: ":keepalive\n"]} = read_chunk(resp)
      assert {:ok, [data: ":keepalive\n"]} = read_chunk(resp)

      # The work finishes and the result lands on the same stream.
      send(worker, :finish)

      assert [%{"result" => %{"content" => [%{"text" => "done blocking"}]}}] =
               resp |> stream_chunks() |> sse_payloads()
    end

    test "start_stream/1 then send_progress/4 produce a single stream" do
      token = "start-stream-token"

      expect_request(fn %MCP.CallToolRequest{id: 3}, channel, _state ->
        assert :ok = Channel.start_stream(channel)
        assert :ok = Channel.send_progress(channel, 1, 1, "half way")
        {:result, MCP.call_tool_result(text: "both")}
      end)
      ...
      assert [
               %{"method" => "notifications/progress", "params" => %{"message" => "half way"}},
               %{"id" => 3, "result" => %{"content" => [%{"text" => "both"}]}}
             ] = sse_payloads([resp.body])
    end

    test "repeat start_stream/1 calls open the stream once" do
      expect_request(fn %MCP.CallToolRequest{id: 4}, channel, _state ->
        assert :ok = Channel.start_stream(channel)
        assert :ok = Channel.start_stream(channel)
        assert :ok = Channel.start_stream(channel)
        {:result, MCP.call_tool_result(text: "once")}
      end)
      ...
      assert ["text/event-stream" <> _] = resp.headers["content-type"]
      assert [%{"id" => 4, "result" => _}] = sse_payloads([resp.body])
    end
  end
