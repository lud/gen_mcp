defmodule GenMCP.Suite.SubscriptionsTest do
  ...
  describe "subscriptions/listen — streaming" do
    test "forwards stream messages to the handler's handle_message/4, stamping subscriptionId" do
      ...
      # The teardown result is a result like any other: it also carries the
      # server info stamped by the Suite, alongside the subscriptionId.
      assert %MCP.Implementation{name: "Test Server", version: "0"} =
               Map.get(result._meta, :"io.modelcontextprotocol/serverInfo")
    end
