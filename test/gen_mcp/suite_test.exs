...
defmodule GenMCP.SuiteTest do
  ...
  # The result as the client receives it: the Suite stamps the server info into
  # the `_meta` on the way out.
  defp with_server_info(result) do
    %{
      result
      | _meta: %MCP.ResultMetaObject{
          "io.modelcontextprotocol/serverInfo": %MCP.Implementation{
            name: "Test Server",
            version: "0"
          }
        }
  ...
  describe "server/discover" do
    ...
    test "serverInfo carries the configured title" do
      ...
      assert {:result, %MCP.DiscoverResult{_meta: meta}} =
               Suite.handle_request(discover_req(), build_channel(), state)

      assert %MCP.Implementation{title: "The Test Server"} =
  ...
  describe "lazy provider resolution" do
    ...
    test "prompts/get for a config repo invokes no extension provider" do
      ...
      expected = with_server_info(result)
  ...
  describe "server info on results" do
    # Servers SHOULD identify themselves on every response: the Suite stamps
    # the `io.modelcontextprotocol/serverInfo` metadata (built from the
    # `:server_name`, `:server_version` and `:server_title` options) into the
    # `_meta` of every result. `send_server_info: false` turns the stamping
    # off.

    test "stamps the server info into a result built without _meta" do
      ToolMock
      |> stub(:info, fn :name, :info_tool -> "InfoTool" end)
      |> expect(:call, fn _req, _channel, :info_tool ->
        {:result, MCP.call_tool_result(text: "hi")}
      end)

      state = init_suite(tools: [{ToolMock, :info_tool}], server_title: "The Test Server")

      assert {:result, %MCP.CallToolResult{_meta: meta}} =
               Suite.handle_request(call_tool_req("InfoTool"), build_channel(), state)

      assert %MCP.ResultMetaObject{
               "io.modelcontextprotocol/serverInfo": %MCP.Implementation{
                 name: "Test Server",
                 version: "0",
                 title: "The Test Server"
               }
             } = meta
    end

    test "fills the serverInfo of a handler-provided result meta" do
      ToolMock
      |> stub(:info, fn :name, :info_tool -> "InfoTool" end)
      |> expect(:call, fn _req, _channel, :info_tool ->
        result = %{MCP.call_tool_result(text: "hi") | _meta: %MCP.ResultMetaObject{}}
        {:result, result}
      end)

      state = init_suite(tools: [{ToolMock, :info_tool}])
    ...
    test "keeps a serverInfo set by the handler" do
      custom = %MCP.Implementation{name: "Custom", version: "9"}

      ToolMock
      |> stub(:info, fn :name, :info_tool -> "InfoTool" end)
      |> expect(:call, fn _req, _channel, :info_tool ->
        meta = %MCP.ResultMetaObject{"io.modelcontextprotocol/serverInfo": custom}
        {:result, %{MCP.call_tool_result(text: "hi") | _meta: meta}}
      end)
      ...
      assert %MCP.ResultMetaObject{"io.modelcontextprotocol/serverInfo": ^custom} = meta
    end

    test "adds the server info to a plain map _meta, keeping its entries" do
      ToolMock
      |> stub(:info, fn :name, :info_tool -> "InfoTool" end)
      |> expect(:call, fn _req, _channel, :info_tool ->
        {:result, %{MCP.call_tool_result(text: "hi") | _meta: %{"com.example/custom" => 1}}}
      end)
      ...
      assert %{
               "com.example/custom" => 1,
               "io.modelcontextprotocol/serverInfo": %MCP.Implementation{name: "Test Server"}
             } = meta
    end

    test "send_server_info: false returns results as the handler built them" do
      ToolMock
      |> stub(:info, fn :name, :info_tool -> "InfoTool" end)
      |> expect(:call, fn _req, _channel, :info_tool ->
        {:result, MCP.call_tool_result(text: "hi")}
      ...
      state = init_suite(tools: [{ToolMock, :info_tool}], send_server_info: false)

      assert {:result, %MCP.CallToolResult{_meta: nil}} =
               Suite.handle_request(call_tool_req("InfoTool"), build_channel(), state)

      # server/discover keeps identifying the server: its result builder
      # carries the serverInfo on its own.
      assert {:result, %MCP.DiscoverResult{_meta: meta}} =
               Suite.handle_request(discover_req(), build_channel(), state)
